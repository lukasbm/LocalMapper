...
def label_retro_edit_site(products, reactants, edit_num):
    ...
    used_atoms = set(
        grow_atoms + [atom for bond in broken_bonds + changed_bonds for atom in bond]
    )
...
def label_foward_edit_site(reactants, products, edit_num):
    ...
    for a in edit_num:
        for b in edit_num:
            ...
            if check_bond_formed(rbond, pbond):  # cut bond
                ...
                if (
                    a not in acceptors1
                    and b not in acceptors1
                    and a not in acceptors2
                    and b not in acceptors2
                ):
    ...
    used_atoms = set(
        [atom for bond in formed_bonds + broken_bonds + changed_bonds for atom in bond]
    )
    remote_atoms = [atom for atom in edit_num if atom not in used_atoms]
    remote_bonds = []
    for a in remote_atoms:
...
def label_CH_change(smiles1, smiles2, edit_num, replacement_dict):
    ...
    for atom_map in edit_num:
        ...
        if atom_map in atom_map_dict2:
            atom1, atom2 = (
                mol1.GetAtomWithIdx(atom_map_dict1[atom_map]),
                mol2.GetAtomWithIdx(atom_map_dict2[atom_map]),
            )
    ...
    H_change = {
        replacement_dict[k]: v["smiles2"] - v["smiles1"] for k, v in H_dict.items()
    }
    Charge_change = {
        replacement_dict[k]: v["smiles2"] - v["smiles1"] for k, v in C_dict.items()
    }
...
def bondmap2idx(bond_maps, idx_dict, temp_dict, remote=False):
    bond_idxs = [
        (idx_dict[bond_map[0]], idx_dict[bond_map[1]]) for bond_map in bond_maps
    ]
    if remote:
        bond_temps = list(set([(temp_dict[bond_map[0]], -1) for bond_map in bond_maps]))
    else:
        bond_temps = [
            (temp_dict[bond_map[0]], temp_dict[bond_map[1]]) for bond_map in bond_maps
        ]
...
def match_label(
    reactants, products, replacement_dict, edit_num, retro=True, remote=True
):
    if retro:
        smiles1 = products
        smiles2 = reactants
    ...
    atom_map_dict, H_change, Charge_change = label_CH_change(
        smiles1, smiles2, edit_num, replacement_dict
    )

    if retro:
        ALG_atoms, broken_bonds, changed_bonds, remote_atoms = label_retro_edit_site(
            smiles1, smiles2, edit_num
        )
        edits = {
        ...
        formed_bonds, broken_bonds, changed_bonds, remote_bonds = (
            label_foward_edit_site(smiles1, smiles2, edit_num)
        )
        edits = {
        ...
        if remote:
            edits["R"] = bondmap2idx(
                remote_bonds, atom_map_dict, replacement_dict, True
            )
