...
from localmapper.LocalTemplate.template_extract_utils import *
from localmapper.LocalTemplate.template_extract_extention import *
...
def inv_temp(template):  # for more canonical representation
    ...
    if len(nums) == 3:
        if nums[0] < nums[2]:
            return template
    bonds = [""] + [
        sorted(bond)[1] for bond in re.findall(r"]([-=#:])|]1([-=#:])", template)
    ]
...
def inverse_template(template):  # for more canonical representation
    n_atoms = sum(
        [Chem.MolFromSmarts(smarts).GetNumAtoms() for smarts in template.split(">>")]
    )
    ...
    if (
        len(bonds1) == 0
        or ")" in template
        or score_bonds(bonds1) <= score_bonds(bonds2)
    ):
        return template
...
def canonicalize_smarts(smarts):  # for more canonical representation
    ...
    if USE_ATOM_SYMBOL:
        return smarts
    preserved_info = {
        "[#0:%s]" % a.split(":")[-1].split("]")[0]: a
        for a in re.findall(r"\[.*?]", smarts)
    }
    try:
...
def sort_template(transform, atom_dict):  # for more canonical representation
    ...
    for tt in transform.split(">>"):
        ts = []
        for smarts in sorted(
            tt.split("."), key=lambda s: template_scorer(s, atom_dict)
        ):
            try:
...
def permutations(template):
    ...
    labels = re.findall("(\[[a-zA-Z@]+\:.*?\])", template)
    if (
        len(labels) == 1 or "(" in template or n_atoms > len(labels)
    ):  # include leaving group
        return [labels]
    charges = re.findall("\;(.+?[0-9]+)\:", template)
    bonds = re.findall("\]([-=#:])\[", template)
    if "".join(bonds) != "".join(bonds[::-1]) or "".join(charges) != "".join(
        charges[::-1]
    ):
        return [labels]
    ...
    for all_labels in p_labels:
        ...
        for label in all_labels:  # keep in order! this is important
            ...
            if atom_map not in replacement_dict:
                replacement_dict_symbol[label] = "%s:%s]" % (
                    label.split(":")[0],
                    counter,
                )
        ...
        transform_newmaps = re.sub(
            "\[[a-zA-Z@]+\:.*?\]", lambda match: replacements.pop(0), transform
        )
        transform_newmaps = ">>".join(transform_newmaps.split(">>")[::-1])

        templates.add(transform_newmaps)
        templates_sort[transform_newmaps] = "".join(
            re.findall("\[[a-zA-Z@]+\:.*?\]", transform_newmaps)
        )
...
def get_fragments_for_changed_atoms(
    mols, changed_atom_tags, category="reactant", special_atoms=[]
):
    fragments = ""
    mols_changed = []
    mols_special = []
    for mol in mols:
                    ...
                    if atom_map in changed_atom_tags:
                        ...
                        if atom.GetIsAromatic():
                            symbol_replacements.append(
                                (atom.GetIdx(), atom.GetSymbol().lower())
                            )
                        else:
                            symbol_replacements.append(
        ...
        if category == "reactant":
            ...
            for atom in mol.GetAtoms():
                if not atom.HasProp(
                    "molAtomMapNumber"
                ):  # LG in reactant is also mapped in foward synthesis
                    atoms_to_use.append(atom.GetIdx())
...
def split_reagents(reaction):
    ...
    largest_p = [
        ...
        for smiles in ps
        if smiles not in rs and smiles != ""
    ]
    if len(largest_p) == 0:
        return [], [], []
    ...
    for smiles in rs:
        ...
        if (
            smiles in ps
            or sum([atom.GetAtomMapNum() in pmaps for atom in mol.GetAtoms()]) == 0
        ):
...
def extract_from_reaction(reaction, setting=default_setting):
    ...
    try:
        reactant_fragments, _, _, reactants, special_atoms = (
            get_fragments_for_changed_atoms(
                reactants, changed_atom_tags, "reactant", []
            )
        )
        product_fragments, _, _, _, _ = get_fragments_for_changed_atoms(
            products, changed_atom_tags, "product", special_atoms
        )
        reaction["reactants"] = ".".join(
            [Chem.MolToSmiles(m, canonical=False) for m in reactants]
        )
