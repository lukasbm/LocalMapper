...
class ReactionDataset(object):
    ...
    def _load_conf_rxns(self, args):
        ...
        print(
            "Load %d templates with total %d reactions from confident predictions"
            % (len(templates_idx), len(conf_df))
        )
        ...
        print(
            "Sampled %d train reaction, %d val reactions"
    ...
    def _make_graphs(self):
        ...
        for i, rxn in tqdm(
            enumerate(self.rxns),
            total=len(self.rxns),
            desc="Generating molecule graphs...",
        ):
