  ...
  for PROFILE in $(budget_profiles_for_dataset "$DATASET_NAME"); do
    ...
    for MODE in $MODES; do
      for SEED_VALUE in $SEEDS; do
        ...
        echo "### Running DATASET=${DATASET} MODE=${INIT_MODE} BUDGET=${BUDGET_LABEL} SAMPLE_LIMIT=${SAMPLE_LIMIT} ITERATIONS=${ITERATIONS} SEED=${SEED} NUM_EPOCHS=${NUM_EPOCHS} MODEL=${MODEL} ###"
