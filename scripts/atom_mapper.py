...
def prediction2map(
    rxn, prediction, fix_product_mapping=False, return_template=True, neighbor_weight=10
):
    reactant, product = rxn.split(">>")
    if fix_product_mapping:
        product_mapping = {
            atom.GetIdx(): atom.GetAtomMapNum()
            for atom in Chem.MolFromSmiles(product).GetAtoms()
        }
    else:
        product_mapping = {
            atom.GetIdx(): atom.GetIdx() + 1
            for atom in Chem.MolFromSmiles(product).GetAtoms()
        }
...
class AtomMapper:
    ...
    def mask_prediction(self):
        self.reactant_mol, self.product_mol = (
            Chem.MolFromSmiles(self.reactant),
            Chem.MolFromSmiles(self.product),
        )
        self.patoms, self.ratoms = (
            Chem.MolFromSmiles(self.product).GetAtoms(),
            Chem.MolFromSmiles(self.reactant).GetAtoms(),
        )
        self.n_patoms, self.n_ratoms = len(self.patoms), len(self.ratoms)
        psymbols, rsymbols = (
            [atom.GetSymbol() for atom in self.patoms],
            [atom.GetSymbol() for atom in self.ratoms],
        )
        self.atom_mask = np.array(
            [[rsymbol == psymbol for rsymbol in rsymbols] for psymbol in psymbols]
        ).astype(int)
    ...
    def _get_normailzed_predictions(self, predictions):
        ...
        column_normalized = np.divide(
            ...
            out=np.zeros_like(predictions),
            where=column_sums[np.newaxis, :] != 0,
        )
    ...
    def _get_combined_normalized_predictions(self):
        combined_predictions = np.multiply(
            self.masked_predictions, self.neighbor_weight_matrix
        )
    ...
    def generate_atom_mapping(self):
        ...
        for i in range(self.n_patoms):
            ...
            corresponding_reactant_atom = np.argmax(normalized_predictions, axis=1)[
                product_atom_to_map
            ]
            ...
            self._update_neighbor_weight_matrix(
            ...
            mapping_steps.append(
                [
                    str(map_number),
                    str(product_atom_to_map),
                    str(corresponding_reactant_atom),
                    round(score, 3),
                ]
            )
            (
                self.product_mapping[int(product_atom_to_map)],
                self.reactant_mapping[int(corresponding_reactant_atom)],
            ) = map_number, map_number

        self.map_steps = pd.DataFrame(
            {t[0]: t[1:] for t in mapping_steps},
            index=["product_idx", "reactant_idx", "score"],
        )
    ...
    def _update_neighbor_weight_matrix(self, product_atom, reactant_atom):
        if reactant_atom != -1:
            ...
            self.neighbor_weight_matrix[
                np.ix_(neighbors_in_products, neighbors_in_reactants)
            ] *= float(self.neighbor_weight)
    ...
    def map_on_rxn(self, reactant_mapping, product_mapping):
        ...
        [
            atom.SetAtomMapNum(reactant_mapping[atom.GetIdx()])
            if atom.GetIdx() in reactant_mapping
            else atom.SetAtomMapNum(0)
            for atom in self.mapped_reactant.GetAtoms()
        ]
        [
            atom.SetAtomMapNum(product_mapping[atom.GetIdx()])
            if atom.GetIdx() in product_mapping
            else atom.SetAtomMapNum(0)
            for atom in self.mapped_product.GetAtoms()
        ]

        return ">>".join(
            Chem.MolToSmiles(smiles, canonical=False)
            for smiles in [self.mapped_reactant, self.mapped_product]
        )
