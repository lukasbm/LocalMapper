...
def load_sampled_data(dataset, chemist_name, samp_iter):
    return pd.read_csv(
        "../data/%s/%s/pred_train_%d.csv" % (dataset, chemist_name, samp_iter)
    )


def save_fixed_data(df, dataset, chemist_name, samp_iter):
    df.to_csv(
        "../data/%s/%s/fixed_train_%d.csv" % (dataset, chemist_name, samp_iter),
        index=None,
    )
    return
...
def load_fixed_templates(dataset, chemist_name, samp_iter):
    pred_temps = load_templates(
        glob.glob("../data/%s/%s/pred_train_*.csv" % (dataset, chemist_name)),
        samp_iter - 1,
    )
    accepted_temps = load_templates(
        glob.glob("../data/%s/%s/fixed_train_*.csv" % (dataset, chemist_name)),
        samp_iter - 1,
    )
    conf_temps = load_templates(
        glob.glob("../data/%s/%s/conf_pred_*.csv" % (dataset, chemist_name)), samp_iter
    )
